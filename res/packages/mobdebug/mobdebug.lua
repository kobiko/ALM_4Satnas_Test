...
local function start(controller_host, controller_port)
  ...
  --small tweaking to avoid timeouts when wrong ip is inserted

  server = socket.tcp()
  server:settimeout(2)
  rval,err  = server:connect(controller_host, controller_port)

  if rval == nil then
     print("Error connecting...".. tostring(err))
  else
      server:settimeout(0)
      if server then
...
local function controller(controller_host, controller_port, scratchpad)
  ...
  server:connect(controller_host, controller_port)
...
--also make mobdebug not local
-- start mobdebug.startDebug() when inserting ip
--mobdebug.lua + debugmenu.lua package
-- socket.lua dependcy of debugmenu package

--[[local function isConnectionAvailable()
    local connection = socket.tcp()
    connection:settimeout(1)
    
    local ipToConnect = debugMenu:getIpFromPersistency()
    print("inside isConnectionAvailable, ipToConnect is "..ipToConnect)
    local result = connection:connect(ipToConnect)
    connection:close()
    print("inside test, returning", tostring(result))
    if (result) then
        print("inside test, returning true")
        return true
    end
    return false
end--]]

mobdebug.startDebug = function()
    if (isrunning()) then return end
    local ipToConnect = debugMenu:getIpFromPersistency()
    print("inside mobdebug.startDebug, ipToConnect is "..ipToConnect)
    if (ipToConnect ~= nil and ipToConnect ~= "") then
            mobdebug.start(ipToConnect)
            print("Lua debugging has started")
...
function startLuaDebugging()
    mobdebug.startDebug()
end

print("mobdebug is loaded")
