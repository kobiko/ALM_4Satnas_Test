...
--
-- usage:
 --local plistStr = itemsConfig.plist
...
function getPackageConfig(package, fileName)
  ...
  local path = "res/packages/"..package.."/"..fileName..".plist"
  local file = io.open(path, "r")
  logstring = file:read("*all")
  
  local configTable = plistParse(logstring)
  
  local mt = {
    __index = configTable,
     __newindex = function(table, key, value)
                    services:debugMessageBox("getPackageConfig:", "attempt to write in config - config is read only!!!")
                  end,
     __metatable = false
     }
  setmetatable(configTable, mt)
  return configTable
